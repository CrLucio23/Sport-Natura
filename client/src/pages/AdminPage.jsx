...
export default function AdminPage() {
  ...
  async function handleConfirm(booking) {
    try {
      ...
      openWhatsApp(
        booking.phone,
        `Ciao ${booking.full_name}! ✅ La tua prenotazione presso Sport&Natura per ${date} è stata *CONFERMATA*.\n\nCi vediamo domenica alle 08:00! 🎯\n\n— Sport&Natura Softair Team`,
      );
    } catch (err) {
      setError(err.message);
    }
  ...
  async function handleRejectSubmit() {
    ...
    try {
      ...
      const date = new Date(rejectModal.booking_date).toLocaleDateString(
        "it-IT",
        {
          weekday: "long",
          day: "2-digit",
      ...
      openWhatsApp(
        rejectModal.phone,
        `Ciao ${rejectModal.full_name}, purtroppo la tua prenotazione per ${date} è stata *RIFIUTATA*.${motivazione}\n\nPer info contattaci su questo numero.\n\n— Sport&Natura Softair Team`,
      );
  ...
  async function handleDelete(id) {
    if (!confirm("Eliminare definitivamente questa prenotazione dal database?"))
      return;
    try {
      setError("");
      setMessage("");
  ...
  async function loadClosedDates() {
    try {
      ...
      // forza nuovo array per triggherare il re-render
  ...
  async function handleToggleDate(date) {
    ...
    const label = new Date(date + "T12:00:00Z").toLocaleDateString("it-IT", {
      weekday: "long",
      day: "2-digit",
      month: "long",
    });
    ...
    const msg = closed
      ? `Vuoi riaprire la domenica del ${label}?`
      : `Sei sicuro di voler chiudere la giocata del ${label}?`;
    ...
    try {
      ...
      // aggiorna ottimisticamente prima del refetch
      if (closed) {
        setClosedDates((prev) => prev.filter((d) => d !== date));
      } else {
        setClosedDates((prev) => [...prev, date]);
      }
      ...
      {rejectModal && (
        ...
        >
          ...
          >
            ...
            >
              {new Date(rejectModal.booking_date).toLocaleDateString("it-IT", {
                weekday: "long",
                day: "2-digit",
                month: "long",
              })}
            ...
            <div className="field-group">
              <label>
                Motivazione (opzionale — verrà inviata via WhatsApp)
              </label>
              <textarea
                rows={3}
                placeholder="es. Campo non disponibile per lavori di manutenzione..."
                value={rejectReason}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    textAlign: "left",
                    border: `1px solid ${closed ? "rgba(140,50,140,0.4)" : "rgba(58,80,52,0.5)"}`,
                    background: closed
                      ? "rgba(75,28,75,0.55)"
                      : "rgba(48,74,42,0.55)",
                    color: "var(--text)",
                ...
                >
                  <div
                    style={{
                      fontSize: "0.68rem",
                      color: closed ? "#c090c0" : "var(--accent)",
                      letterSpacing: "0.1em",
                      textTransform: "uppercase",
                      marginBottom: "0.25rem",
                    }}
      ...
      <div className="admin-list">
        {bookings.map((booking) => (
          <article key={booking.id} className="booking-card">
            ...
            <p>
              <strong>Fascia:</strong> {booking.slot_label} (
            ...
            <p>
              <strong>Noleggi:</strong> {booking.rental_count || 0}
            </p>
            <p>
            ...
            <div className="status-actions">
              ...
              <button
                ...
                style={{
                  ...
                  borderColor: "rgba(140,40,40,0.5)",
                }}
              >
                🗑 Elimina
              </button>
